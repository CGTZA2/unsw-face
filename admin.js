  ...
  app.innerHTML = `
    <section class="screen">
      <div class="body-stack">
        ...
        <article class="panel content-panel">
          <h2 class="screen-title">Asset Upload</h2>
          <p class="screen-subtitle">Upload images and tag them for study, memory, and matching phases.</p>
          <div class="fields">
            <div class="field">
        ...
        <article class="panel content-panel">
          ...
          <div class="copy-box">
            ...
            <p>For matching uploads, assign one target asset and four probe assets to the same <code>trial_set_id</code>.</p>
            <p>Study assets and memory_old assets should share the same <code>identity_id</code> so the old-memory trials can be derived from studied identities.</p>
          </div>
        </article>
      </div>
    </section>
...
function renderAssets() {
  ...
  if (!state.assets.length) {
    container.innerHTML = "<p class='panel-copy'>No assets uploaded yet.</p>";
    return;
  }
  container.innerHTML = `
    <table class="admin-table">
      <thead><tr><th>Label</th><th>Role</th><th>Population</th><th>Identity</th><th>Trial Set</th><th>Preview</th></tr></thead>
      <tbody>
        ${state.assets
          .map((asset) => {
